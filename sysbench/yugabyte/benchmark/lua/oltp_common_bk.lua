...
function init()
 ...
 local stmt_defs = {
    point_selects = {
       ...
       {t.VARCHAR,255}},
    simple_ranges = {
       "SELECT * FROM sbtest%u WHERE FIELD0 BETWEEN ? AND ?",
       {t.INT}, {t.INT}},
    sum_ranges = {
       "SELECT SUM(amount) FROM sbtest%u WHERE FIELD0 BETWEEN ? AND ?",
       {t.INT}, {t.INT}},
    order_ranges = {
       "SELECT * FROM sbtest%u WHERE FIELD0 BETWEEN ? AND ? ORDER BY FIELD1",
       {t.VARCHAR,255}, {t.VARCHAR,255}},
    distinct_ranges = {
       "SELECT DISTINCT FIELD1 FROM sbtest%u WHERE FIELD0 BETWEEN ? AND ? ORDER BY FIELD1",
       {t.INT}, {t.INT}},
    index_updates = {
 ...
 local function execute_range(key)
    ...
    for i = 1, sysbench.opt[key] do
       ...
       param[tnum][key][1]:set(id_num)
       -- param[tnum][key][2]:set(id + sysbench.opt.range_size - 1)
       param[tnum][key][2]:set(id_num + sysbench.opt.range_size - 1)
